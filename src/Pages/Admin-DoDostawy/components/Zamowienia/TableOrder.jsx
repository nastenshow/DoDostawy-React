...
const TableOrder = () => {
  ...
  const DeliveryTimeButton = ({ time }) => (
    <button className="status-niedostarczono">{time}</button>
  );
  ...
  const fetchOrderDetails = async (orderNumber) => {
    ...
    try {
      ...
      console.log("Order details:", orderDetails); // Переконайтеся, що orderDetails містить rateOutsideZone
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        // Сортування даних за зменшенням номера замовлення
        const sortedData = filteredData.sort((a, b) => {
          return b.order_number - a.order_number; // Сортування за зменшенням
        });
  ...
  const deleteOrder = async (orderNumber) => {
    ...
    const isConfirmed = window.confirm("Czy na pewno chcesz usunąć to zamówienie?");
    if (!isConfirmed) {
    ...
    <>
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.order_number}>
                ...
                <td>
                  {order.delivery_time === "Tak szybko, jak to możliwe" ? (
                    order.delivery_time
                  ) : (
                    <DeliveryTimeButton time={order.delivery_time} />
                  )}
                </td>
      ...
      {isModalVisible && selectedOrder && (
        <ModalComponentEditOrder
          ...
          setOrders={setOrders}
          restaurantId={selectedOrder.order.restaurant_id}
